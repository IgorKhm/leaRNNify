...
def learn_dfa(dfa: DFA, benchmark, hidden_dim=-1, num_layers=-1, embedding_dim=-1, batch_size=-1,
    ...
    if hidden_dim == -1:
        hidden_dim = (len(dfa.states) + len(dfa.alphabet)) * 3
    if num_layers == -1:
        num_layers = int(hidden_dim / 10) + 2
    if embedding_dim == -1:
...
def rand_benchmark(save_dir=None):
    ...
    while (len(dfa_inter.states) < 5 and len(dfa_spec.states) < 2) or len(dfa_inter.states) > 35:
