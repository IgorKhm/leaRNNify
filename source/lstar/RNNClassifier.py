...
from lstar.GRU import GRUNetwork
from lstar.LinearTransform import LinearTransform
import dynet as dy
...
class RNNClassifier:
    ...
    def train_group(self, word_dict, iterations, trainer_class=dy.AdamTrainer, learning_rate=None, loss_every=100,
        ...
        for i in range(iterations):
            ...
            loss_values.append(sum(
                batches_loss) / num_batches)  # its not perfect because the last batch might be a different size and they were training during, but whatever, its here to give a general idea of what's going on
    ...
    def probability_word_in_language(self, word):
        # verification, could get rid of
        if not self._word_is_over_input_alphabet(word):
            print("word is not over input alphabet")
            return False
        s = self.rnn.initial_state
        for c in word:
            s = self.rnn.next_state(s, self._char_to_input_vector(c))
        return self._state_accept_probability(s)
